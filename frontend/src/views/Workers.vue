<template>
  <div class="Workers">
    <BaseHeadline :route="backbutton" :title="title" :description="`Started: ${date}`" :meta="`HIT: ${id}`" />
    <BaseWrapper title="Workers waiting for approval" gray-dark>
      <TableSubmitted :workers="submitted" @onApprove="handleApprove" @onReject="handleReject" />
    </BaseWrapper>
    ...
    <BaseModal :visible="modalApproveIsVisible" title="Approve Assignment" :cancel="{label:'cancel'}" :accept="{label:'approve',type:'success'}" @onAccept="approveAssignment" @onCancel="closeModal">
      <p>Leave feedback for the worker if you like</p>
      <BaseTextarea @keyPress="setApprovalFeedback" label="Feedback" :value="approvalFeedback" />
    </BaseModal>

    <BaseModal :visible="modalRejectIsVisible" title="Reject Assignment" :cancel="{label:'cancel'}" :accept="{label:'reject',type:'warning'}" @onAccept="rejectAssignment" @onCancel="closeModal">
      <p>Leave feedback for the worker</p>
      <BaseTextarea @keyPress="setRejectionFeedback" label="Feedback" :value="rejectFeedback" />
    </BaseModal>
  </div>
...
export default {
  ...
  data: () => ({
    ...
    rejectFeedback: "Worker did not submit any data, our server shows no HIT-relevant activity whatsoever for this assigment.",
    ...
    async getWorkers() {
      ...
      if (res.success) {
        ...
        for (let assignment of assignments) {
          ...
          if (status === 'rejected') {
            let rejectionTime = moment(assignment.RejectionTime).format("HH:mm:ss");
            let rejectionDate = moment(assignment.RejectionTime).format("DD.MM.YYYY");
            worker.rejected = {
              time: rejectionTime,
              date: rejectionDate
            }
            ...
            let approvedTime = moment(assignment.ApprovalTime).format("HH:mm:ss");
            let approvedDate = moment(assignment.ApprovalTime).format("DD.MM.YYYY");
            worker.approved = {
              time: approvedTime,
              date: approvedDate
            }
