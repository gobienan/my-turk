<template>
  <div class="Overview">
    <BaseHeadline :route="backbutton" prime title="myTurk" description="Overview of your experiments" />

    <BaseWrapper title="Production" red>
      <Table :experiments="experiments.production" @createHIT="createHIT" @onHitDeleteClick="handleDeleteHIT" />
    </BaseWrapper>

    <BaseWrapper title="Sandbox" green>
      <Table :experiments="experiments.sandbox" @createHIT="createHIT" @onHitDeleteClick="handleDeleteHIT" />
    </BaseWrapper>
    ...
    <BaseModal :visible="modalIsVisible" title="Delete HIT" :cancel="{label:'cancel'}" :accept="{label:'delete',type:'warning'}" @onAccept="deleteHIT" @onCancel="closeModal">
...
export default {
  ...
  methods: {
    ...
    async addExperiment() {
      ...
      if (res.success) {
        this.$router.push({ name: 'settings', params: { addExperiment: true }, query: { id: res.data.id } });
  ...
  mounted: async function(argument) {
    this.getExperiments();
  }
};
